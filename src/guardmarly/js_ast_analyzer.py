...
from guardmarly.js_engine import (
    JsCall,
    JsPropertyWrite,
    append_trace,
    collect_calls,
...
from guardmarly.js_engine.react import run_react_checks
from guardmarly.js_engine.routes import run_route_checks
from guardmarly.js_engine.taint_checks import run_taint_flow_checks
from guardmarly.js_engine.minified_scanner import scan_minified_js
...
def _run_structural_checkers(
    ...
    try:
        ...
        _log.debug("guardmarly: syntax-aware JS analysis failed on %r: %s", filename, exc, exc_info=True)
...
def _rust_fast_path(code: str, filename: str) -> AnalysisResult | None:
    ...
    try:
        from guardmarly.engine.rust_parser import HAS_RUST_CORE, fast_parse
    except ImportError:
        return None
...
def analyze_js_ast(code: str, filename: str = "", global_graph: object | None = None) -> AnalysisResult:
    ...
    try:
        from guardmarly.engine.symbolic_guards import analyze_guards_js
