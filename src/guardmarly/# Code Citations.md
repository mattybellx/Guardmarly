...
# Complete AI Implementation Guide — Guardmarly v6.0
...
Regex without AST confirmation should never be displayed as high-confidence. Add to `src/guardmarly/engine/confidence.py`:
...
from guardmarly.engine.confidence import cap_regex_only_findings
...
And in the `runs:` section where `guardmarly` is invoked, add `--min-confidence ${{ inputs.min-confidence }}`.

---

## PART 2: LIVE PLAYGROUND — The Conversion Multiplier

**Goal:** Add a `/scan` endpoint to the existing Flask webapp so visitors can try Guardmarly without installing anything.
...
<title>Guardmarly — Live Playground</title>
<style>
...
<header>
  ...
  <a href="https://github.com/mattybellx/Guardmarly" target="_blank" style="margin-left:auto; font-size:13px; color:#8b949e;">⭐ Star on GitHub</a>
</header>
<div class="container">
  <div class="panel">
  ...
  <div class="panel">
    ...
    <div class="results" id="resultsPanel">
      <div class="placeholder">
        ...
        <br>
        <p>Powered by <code>guardmarly</code> — 100% CVE recall · fully offline · no data leaves this server</p>
      </div>
    </div>
  </div>
</div>
...
function renderResults(data) {
  ...
  if (findings.length === 0) {
    panel.innerHTML = `<div class="zero-findings"><div class="check">✅</div><p>No security issues found</p><small>${data.lines_scanned} lines scanned · Try the examples above to see Guardmarly in action</small></div>`;
    return;
  }
