...
guardmarly.cli
─────────────────
Command-line interface for guardmarly.

    guardmarly path/to/file.py
    guardmarly src/ --format sarif --output results.sarif
...
def _matches_exclude_pattern(path: Path, pattern: str) -> bool:
    ...
    This avoids false positives such as excluding ``guardmarly`` when the
...
def _load_guardmarlyignore(workspace_root: Path) -> list[str]:
    """Load .guardmarlyignore patterns from the workspace root (gitignore-compatible syntax).

    Blank lines and #-comments are skipped; negations (!pattern) are not yet supported.
    """
    ignore_file = workspace_root / ".guardmarlyignore"
...
def _analyze_entropy_file(path: Path) -> AnalysisResult:
    ...
    try:
        from guardmarly.entropy import scan_for_secrets
...
def _render_rule_catalog(as_json: bool) -> str:
    ...
    lines = ["guardmarly rule catalog", "-" * 72]
    for rule in rules:
...
def _render_js_backend_catalog(as_json: bool) -> str:
    ...
    lines = ["guardmarly JS backend catalog", "-" * 72]
...
def _postprocess_guarded_rescan_results(
    ...
    *,
    ...
    from guardmarly.engine.triage import reduce_confidence_for_traced_sanitizer
    for ar in processed:
    ...
    from guardmarly.engine.entry_points import apply_entry_point_triage
...
def _handle_baseline_command(args: list[str]) -> None:
    """Handle ``guardmarly baseline generate`` and ``guardmarly baseline load`` subcommands."""
    if not args or args[0] == "--help":
        print(textwrap.dedent("""\
            Usage: guardmarly baseline <command> [options]
    ...
    if cmd == "generate":
        ...
        print(f"guardmarly: unknown baseline command: {cmd}", file=sys.stderr)
        sys.exit(2)


def _handle_license_command(args: list[str]) -> None:
    """Handle ``guardmarly license`` — view status, activate a key, or show pricing."""
    if not args:
        # Show current license status
        info = load_license()
        print("guardmarly License Status")
        print("=" * 40)
    ...
    if cmd == "activate":
        if len(args) < 2:
            print("Usage: guardmarly license activate <license-key>", file=sys.stderr)
            sys.exit(2)
        key = args[1]
        ...
        import webbrowser
        print()
        print("  Opening guardmarly.onrender.com in your browser...")
        print()
        print("  💸  One-time £4.99  —  30 days Pro access")
        print("  ⭐  Pro £49/year    —  everything included")
        print()
        webbrowser.open("https://guardmarly.onrender.com")
    elif cmd in ("--help", "-h", "help"):
        print("Usage: guardmarly license [command]")
        print()
        print("Commands:")
        ...
        print()
        print("  Visit https://guardmarly.onrender.com to purchase a license.")
    else:
...
def _handle_migrate_config_command(args: list[str]) -> None:
    """Handle ``guardmarly migrate-config`` — converts v1 to v2 configuration format."""
    if not args or args[0] == "--help":
        print(textwrap.dedent("""\
            Usage: guardmarly migrate-config [--input FILE] [--output FILE]

            Upgrade an guardmarly.json from v1 format to v2 format (spec §4.1).

            Examples:
              guardmarly migrate-config --input guardmarly.json --output guardmarly-v2.json
              guardmarly migrate-config  # in-place upgrade of guardmarly.json
    ...
    parser = argparse.ArgumentParser(prog="guardmarly migrate-config")
    parser.add_argument("--input", "-i", type=Path, default=Path("guardmarly.json"), metavar="FILE")
    parser.add_argument("--output", "-o", type=Path, default=None, metavar="FILE")
    parsed_args = parser.parse_args(args)

    if not parsed_args.input.is_file():
        print(f"guardmarly: config file not found: {parsed_args.input}", file=sys.stderr)
        sys.exit(2)

    try:
        data = json.loads(parsed_args.input.read_text(encoding="utf-8"))
    except json.JSONDecodeError as exc:
        print(f"guardmarly: {parsed_args.input} is not valid JSON: {exc}", file=sys.stderr)
        sys.exit(2)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=textwrap.dedent("""\
            Examples:
              ...
              guardmarly src/ --format json --output report.json
              guardmarly src/ --format sarif --output results.sarif
              guardmarly --stdin --lang python < app.py
              guardmarly src/ --fail-on high
            ...
            SARIF output is free. Upgrade to Pro for SBOM & HTML dashboards:
              ...
              Get a key: https://guardmarly.onrender.com
        """),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Run Semgrep as a secondary scanner (if installed) and merge its findings into the output. "
             ...
             "Findings overlapping with Guardmarly's native detections are automatically deduplicated.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Start guardmarly as an LSP server on stdio for IDE integration.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(does not require a git repository).  Cache stored in .guardmarly/cache.db."
        ),
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "Audit all `# guardmarly: ignore` comments in the scanned paths. "
    ...
    parser.add_argument(
        ...
        help="Path to golden corpus directory for rule validation (default: .guardmarly/golden_corpus).",
    )
    parser.add_argument(
        "--demo", action="store_true",
        help="Scan a built-in vulnerable code sample to see what Guardmarly findings look like.",
    )
    return parser
...
def _get_version_str() -> str:
    ...
    return f"guardmarly {get_engine_version()}"


def _print_config_warnings(warnings: list[str]) -> None:
    for warning in warnings:
        msg = f"guardmarly: config warning: {warning}"
        if console:
...
def _handle_feedback(args: argparse.Namespace) -> None:
    """Record a false-positive entry to ~/.guardmarly/feedback.jsonl."""
    import datetime
    feedback_dir = Path.home() / ".guardmarly"
...
def _analyze_file_with_timeout(
    ...
    *,
    ...
    t = threading.Thread(target=_worker, daemon=True, name=f"guardmarly-{path.name}")
    t.start()
    try:
...
def main() -> None:
    ...
    try:
        ...
        # stdout closed (e.g., `guardmarly ... | head`); no stack trace needed
        sys.stderr.close()
        sys.exit(0)
...
def _run_dse_validation(golden_corpus_path: Path | None) -> None:
    ...
    if not rules:
        print("guardmarly: DSE validation — no community rules found.")
        print("  Place single-rule YAML files in ~/.guardmarly/community_rules/")
        sys.exit(0)

    print(f"guardmarly: DSE validation — {len(rules)} community rule(s)")
...
This is the built-in example for `guardmarly --demo`.
"""
...
def _run_demo(*, colour: bool = True) -> None:
    ...
    their first interaction with Guardmarly, regardless of whether
    ...
    if console and colour:
        console.print()
        console.print("[bold cyan]guardmarly --demo[/bold cyan]")
        ...
        if result.findings:
            ...
            from guardmarly.reporters import format_text
        ...
        console.print("  [cyan]guardmarly src/[/cyan]")
        console.print()
...
def _run_watch_mode(paths: list[Path], *, args: argparse.Namespace, interval: float = 1.5) -> None:
    ...
    _msg = f"guardmarly --watch: watching {len(all_files)} file(s) in {_label}. Ctrl+C to stop."
    if console:
...
def _run_audit_suppressions(paths: list[Path], *, exclude: list[str]) -> None:
    """Audit all # guardmarly: ignore suppression comments in scanned files.
    ...
    for fp, ln in broad:
        if console:
            console.print(f"  {fp}:{ln}  [bold red]BROAD[/bold red]     # guardmarly: ignore  (no rule ID \u2014 consider specifying one)")
        else:
            print(f"  {fp}:{ln}  BROAD     # guardmarly: ignore  (no rule ID \u2014 consider specifying one)")
...
def _main_impl() -> None:
    ...
    if len(sys.argv) >= 2 and sys.argv[1] == "registry":
        from guardmarly.registry import handle_registry_command
    ...
    if isinstance(getattr(args, "explain", None), str) and getattr(args, "explain", None) not in {"", "__INLINE__"}:
        ...
        if rendered is not None:
            print(rendered)
            from guardmarly.engine.explain import EXPLANATIONS
        ...
        if explanation is None:
            print(f"guardmarly: unknown explain token: {token}", file=sys.stderr)
            sys.exit(2)
        print(explanation)
        sys.exit(0)
    ...
    if getattr(args, "export_rules", False):
        ...
        if rules_output_path:
            ...
            print(f"guardmarly: rule catalog written to {rules_output_path}")
        else:
            print(rendered)
        sys.exit(0)
    ...
    if getattr(args, "describe_rule", None):
        ...
        if rendered is None:
            print(f"guardmarly: unknown rule token: {args.describe_rule}", file=sys.stderr)
            sys.exit(2)
        print(rendered)
        sys.exit(0)
    ...
    if getattr(args, "explain_cwe", None):
        ...
        if explanation is None:
            print(f"guardmarly: no explanation available for {cwe_id}", file=sys.stderr)
            sys.exit(2)
        print(explanation)
        sys.exit(0)
    ...
    if getattr(args, "lsp", False):
        from guardmarly.lsp_server import run_lsp_server
        run_lsp_server()
        sys.exit(0)

    # ── Handle Init ────────────────────────────────────────────────────────
    if getattr(args, "init", False):
        init_file = Path.cwd() / "guardmarly.json"
        if init_file.exists():
...
''')
    ...
    if getattr(args, "openapi_report", False):
        from guardmarly.graph.openapi_bridge import cli_bridge_report
        root = Path(".").resolve()
        if args.paths:
    ...
    try:
        ...
        print(f"guardmarly: warning: custom/community rules error: {exc}", file=sys.stderr)
    ...
    with temporary_analyzer_config(config):
        ...
        if args.incremental:
            ...
            try:
                ...
                try:
                    from guardmarly.monorepo import detect_monorepo
            ...
            # ── Load .guardmarlyignore patterns ─────────────────────────────────
            _guardmarlyignore_patterns = _load_guardmarlyignore(workspace_root)
            ...
            if not files and not entropy_files:
                if console:
                    console.print(
                        f"[bold red]guardmarly: no supported source files found in: {', '.join(str(p) for p in args.paths)}[/bold red]"
                    )
                else:
                    print(
                        f"guardmarly: no supported source files found in: {', '.join(str(p) for p in args.paths)}",
                        file=sys.stderr,
                    )
                sys.exit(2)
            ...
            if getattr(args, "batch", False) and not getattr(args, "entropy", False):
                ...
                if console:
                    ...
                    print(f"guardmarly: batch mode: scanning {len(files)} files with {_batch_n_workers} workers", file=sys.stderr)
                n_workers = _batch_n_workers
                from guardmarly import scan_files as _batch_scan_files
            ...
            if getattr(args, "incremental_sha256", False) and not _batch_mode_done:
                try:
                    ...
                    if cached_unchanged:
                        print(
                            f"guardmarly: SHA-256 cache: {len(cached_unchanged)} unchanged "
                    ...
                    print(f"guardmarly: incremental cache warning: {_exc}", file=sys.stderr)
                    _inc_cache = None
        ...
        if files:
            if scan_targets:
                if use_parallel:
                    ...
                    for idx, fpath in enumerate(scan_targets, 1):
                        print(f"guardmarly: scanning [{idx}/{total}] {fpath.name}", file=sys.stderr)
                        results.append(_scan_one(fpath))
            ...
            if _profiler is not None:
                ...
                if profile_path:
                    ...
                    print(f"guardmarly: profile written to {profile_out}", file=sys.stderr)
                else:
    ...
    if _yaml_rules is not None:
        for r in results:
            ...
            try:
                ...
                print(f"guardmarly: warning: custom rules error: {exc}", file=sys.stderr)
    ...
    if _test_downgraded > 0:
        print(f"guardmarly: test/benchmark/tutorial context downgraded {_test_downgraded} findings",
              file=sys.stderr)
    ...
    if getattr(args, "strict", False):
        ...
        if _strict_filtered > 0:
            print(f"guardmarly: --strict filtered {_strict_filtered} findings, kept {_strict_kept} high+critical",
                  file=sys.stderr)
    ...
    if getattr(args, "ai_remediate", False):
        from guardmarly.engine.remediation import generate_remediation
    ...
    if diagnostics_enabled and results:
        ...
        if args.diagnostics_output is not None:
            ...
            if not args.quiet:
                print(
                    f"guardmarly: diagnostics written to {args.diagnostics_output}",
                    file=sys.stderr,
                )
    ...
    if getattr(args, "cluster", True):
        try:
            from guardmarly.engine.triage import cluster_results
        ...
        try:
            ...
            if getattr(args, "auto_rule", False):
                try:
                    ...
                    print(f"guardmarly: auto-rule generation error: {exc}", file=sys.stderr)
            ...
            if getattr(args, "apply_auto_rules", False):
                try:
                    ...
                    msg = f"guardmarly: applied {len(loaded_rules)} saved auto-rule(s) during audit"
                    if console:
                        console.print(f"[bold green]{msg}[/bold green]")
                    else:
                        print(msg, file=sys.stderr)
                except Exception as exc:
                    print(f"guardmarly: auto-rule application error: {exc}", file=sys.stderr)
            ...
            if getattr(args, "llm", False):
                try:
                    ...
                    if check_ollama_available(model):
                        ...
                        msg = f"guardmarly: LLM-triaged audit written to {audit_path}"
                        if console:
                            console.print(f"[bold green]{msg}[/bold green]")
                        else:
                            print(msg, file=sys.stderr)
                    else:
                        msg = f"guardmarly: Ollama not available or model '{model}' not found. Install with: ollama pull {model}"
                        if console:
                            console.print(f"[bold yellow]{msg}[/bold yellow]")
                        else:
                            print(msg, file=sys.stderr)
                except ImportError:
                    print("guardmarly: llm_triage module not available", file=sys.stderr)
                except Exception as exc:
                    print(f"guardmarly: LLM triage error: {exc}", file=sys.stderr)

            audit_report.export_json(audit_path)
            msg = f"guardmarly: audit report written to {audit_path}"
            if console:
                console.print(f"[bold green]{msg}[/bold green]")
            else:
                print(msg, file=sys.stderr)
            ...
            import traceback
            print(f"guardmarly: audit pipeline error: {exc}", file=sys.stderr)
            traceback.print_exc()
    ...
    if getattr(args, "cross_language", False):
        if args.stdin:
            ...
            try:
                ...
                msg = (
                    "guardmarly: cross-language graph built "
                ...
                msg = f"guardmarly: cross-language graph build failed: {exc}"
                if console:
                    console.print(f"[bold yellow]{msg}[/bold yellow]")
                else:
                    print(msg, file=sys.stderr)
    ...
    if getattr(args, "with_semgrep", False):
        try:
            ...
            if is_available():
                ...
                if not args.quiet:
                    print(
                        f"guardmarly: semgrep: {_added} supplementary findings merged",
                        file=sys.stderr,
                    )
            else:
                if not args.quiet:
                    print("guardmarly: --with-semgrep: semgrep not found, skipping", file=sys.stderr)
        except Exception:
            pass
    ...
    if getattr(args, "cluster", False):
        try:
            from guardmarly.engine.clustering import cluster_findings
            ...
            if not args.quiet:
                print(
                    f"guardmarly: incident clustering: {_raw_total} → {_clustered_total} findings "
    ...
    if args.format == "text":
        ...
        if diagnostics_payload is not None:
            try:
                parsed = json.loads(output)
                parsed["guardmarly_diagnostics"] = diagnostics_payload
                output = json.dumps(parsed, indent=2)
            except (json.JSONDecodeError, TypeError):
                pass
    ...
    if primary_output_path:
        try:
            ...
            if args.format == "text":
                ...
                msg = f"guardmarly: {total} findings written to {primary_output_path}"
                if console:
                    console.print(f"[bold green]✓[/bold green] {msg}")
                else:
                    print(msg)
        except OSError as exc:
            msg = f"guardmarly: cannot write to {primary_output_path}: {exc}"
            if console:
    ...
    if getattr(args, "sbom", None):
        try:
            from guardmarly import sbom as _sbom
            ...
            print(f"guardmarly: SBOM generation failed: {exc}", file=sys.stderr)
    # ── PR document generation ──────────────────────────────────────────────
    want_pr = getattr(args, "pr", False) or getattr(args, "pr_output", None) is not None
    if want_pr:
        try:
            from guardmarly.engine.pr_generator import write_pr_document
