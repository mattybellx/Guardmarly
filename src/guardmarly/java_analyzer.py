"""
java_analyzer.py — Guardmarly Java detection engine.
...
try:
    ...
    import warnings
    warnings.warn(
        "guardmarly: tree-sitter-java not installed — Java analysis "
        "will use regex fallback (reduced accuracy). Install with: "
        "pip install guardmarly[treesitter]",
        RuntimeWarning,
        stacklevel=2,
    )
...
def analyze_java(
    ...
    *,
    ...
    if global_graph is not None:
        try:
            from guardmarly.ir.global_graph import FunctionSummary
            for f in findings:
