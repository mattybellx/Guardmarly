...
─────
    ...
    cache = IncrementalCache()          # uses .guardmarly/cache.db by default
    # or:
...
class IncrementalCache:
    ...
    ----------
    db_path
        Path to the SQLite database.  Defaults to ``.guardmarly/cache.db``
        relative to the current working directory.
    """

    def __init__(self, db_path: Optional[str | Path] = None) -> None:
        if db_path is None:
            db_path = Path(".guardmarly") / "cache.db"
