"""
java_ast_analyzer.py — Guardmarly tree-sitter Java analysis engine.
...
def _check_sqli(methods: list[_JavaMethod], source: bytes) -> list[Finding]:
    ...
    from guardmarly.java_taint_origins import collect_taint_origins, has_user_origin
    ...
    if len(methods) > 1:
        ...
        if _has_cross_calls:
            ...
            if cache_key in _IFDS_CACHE:
                ...
                try:
                    from guardmarly.v2_java_bridge import run_interprocedural_ifds
    ...
    for method in methods:
        ...
        if _has_any_sinks_for_ifds and (not ifds_all_findings or method.name not in ifds_all_findings):
            try:
                from guardmarly.v2_java_bridge import run_ifds_analysis
        ...
        if _has_sql_sinks and _has_taint_sources:
            try:
                from guardmarly.java_dataflow import run_intraprocedural_dataflow
        ...
        from guardmarly.statement_tracker import classify_statement_variables, is_safe_sql_call
