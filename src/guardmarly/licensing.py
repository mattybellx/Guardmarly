"""
guardmarly.licensing
───────────────────────
Offline license key system for guardmarly.
...
License keys are issued by the guardmarly licensing server and verified
...
_LICENSE_SERVER = os.environ.get("GUARDMARLY_LICENSE_SERVER", "https://guardmarly.dev")
...
# This is the OFFICIAL guardmarly licensing public key.
# Private key is held securely by the guardmarly licensing server.
...
def _decode_license_key(key: str) -> dict[str, Any] | None:
    ...
    try:
        ...
        if header.get("alg") != "HS256" or header.get("typ") != "GUARDMARLY-LIC":
            return None
...
def _generate_free_tier_license() -> LicenseInfo:
    ...
    return LicenseInfo(
        tier="free",
        licensee="free@guardmarly.dev",
        seats=1,
        issued_at=1715875200,
        expires_at=0,
        key_id="free-built-in",
        raw_payload={
            "sub": "free@guardmarly.dev",
            "tier": "free",
            "iat": 1715875200,
            "exp": 0,
            "seats": 1,
...
def _license_file_path() -> Path:
    ...
    # Check GUARDMARLY_LICENSE_KEY env var first
    import os
    env_key = os.environ.get("GUARDMARLY_LICENSE_KEY", "")
    if env_key:
        return Path(".guardmarly-license-key")  # temporary, will be read from env

    # Windows: %APPDATA%\guardmarly\license.key
    # Linux/macOS: ~/.config/guardmarly/license.key
    if os.name == "nt":
        base = Path(os.environ.get("APPDATA", Path.home() / "AppData" / "Roaming"))
    else:
        base = Path(os.environ.get("XDG_CONFIG_HOME", Path.home() / ".config"))
    return base / "guardmarly" / "license.key"
...
def save_license_key(key: str) -> LicenseInfo | None:
    ...
    if now < cooldown_until:
        ...
        print(f"guardmarly: too many activation attempts. Try again in {remaining // 60}m {remaining % 60}s.", file=__import__('sys').stderr)
        return None
...
def format_license_status(license_info: LicenseInfo | None = None) -> str:
    ...
    if license_info.tier == "free":
        ...
        lines.append("")
        lines.append("  Visit https://guardmarly.onrender.com to upgrade.")
...
class LicenseFeatureGate:
    ...
    def require_or_raise(self, feature: str, feature_name: str = "") -> str:
        ...
        msg = (
            ...
            f"  ║                                                      ║\n"
            f"  ║  Run: guardmarly license upgrade                   ║\n"
            f"  ║  Or visit: https://guardmarly.onrender.com                 ║\n"
            f"  ╚══════════════════════════════════════════════════════════╝\n"
        )
...
def _lifetime_count_file() -> Path:
    return Path.home() / ".guardmarly" / "lifetime_scan_count.json"


def _lifetime_hash_file() -> Path:
    return Path.home() / ".guardmarly" / ".lifetime_scan_integrity"
