"""
guardmarly.v2.suppression
─────────────────────────────
Inline suppression comment parsing (Phase 6 §6.1).

Suppression format:
    # guardmarly: ignore PY-SEC-001 -- reason text
    # guardmarly: ignore PY-SEC-001,PY-SEC-002
    # guardmarly: ignore              ← bare ignore (warns; suppresses all rules on that line)

Rules:
  - A bare `# guardmarly: ignore` with no rule IDs is accepted but emits a
...
# Matches:  # guardmarly: ignore [optional IDs] [-- optional reason]
_SUPPRESSION_RE = re.compile(
    r"#\s*guardmarly\s*:\s*ignore\s*(?P<ids>[^-\n#]*)(?:--(?P<reason>.*))?",
    re.IGNORECASE,
)
...
def parse_suppressions(source: str, file_path: str = "<unknown>") -> dict[int, frozenset[str]]:
    ...
    An empty frozenset means the line carries a bare ``# guardmarly: ignore``
    ...
    for lineno, line in enumerate(source.splitlines(), start=1):
        ...
        if not rule_ids:
            warnings.warn(
                f"{file_path}:{lineno}: bare `# guardmarly: ignore` without a rule ID "
                "suppresses all rules on this line; specify a rule ID like "
                "`# guardmarly: ignore PY-SEC-001` to scope the suppression.",
                StacklevelWarning,
                stacklevel=2,
            )
...
class StacklevelWarning(UserWarning):
    """Emitted for bare (unscoped) guardmarly ignore directives."""
