...
from guardmarly.engine.confidence import rescore_findings
...
from guardmarly._types import Finding, Severity, AnalysisResult, TraceFrame
...
# Inline suppression:  # guardmarly: ignore  |  # guardmarly: ignore[CWE-862]
_SUPPRESSION_RE: re.Pattern[str] = re.compile(
    r'#\s*guardmarly:\s*ignore(?:\[([\w\-,\s]+)\])?', re.IGNORECASE,
)
...
def _load_cached_function_summaries(code: str, filename: str) -> dict[str, _FunctionTaintSummary] | None:
    ...
    try:
        with SQLiteStore(Path(".guardmarly") / "cache.db") as store:
...
def _record_function_summaries_in_global_graph(
    ...
    *,
    ...
    from guardmarly.ir.global_graph import FunctionSummary, IDETaintLevel
...
_GUARDMARLY_INTERNAL_PY_MARKERS: tuple[str, ...] = (
    "src/guardmarly/",
    "/src/guardmarly/",
    "site-packages/guardmarly/",
    "/site-packages/guardmarly/",
)

_GUARDMARLY_INTERNAL_PY_NOISE_RULES: frozenset[str] = frozenset({
...
_GUARDMARLY_INTERNAL_PY_RULE_DOWNGRADE_PATHS: dict[str, tuple[str, ...]] = {
    ...
    "PY-045": (
        "src/guardmarly/cli.py",
        "/src/guardmarly/cli.py",
        "site-packages/guardmarly/cli.py",
        "/site-packages/guardmarly/cli.py",
    ),
}
...
def _is_guardmarly_internal_python_noise_path(rule_id: str, filename: str) -> bool:
    path_norm = filename.replace("\\", "/").lower()
    return any(fragment in path_norm for fragment in _GUARDMARLY_INTERNAL_PY_RULE_DOWNGRADE_PATHS.get(rule_id, ()))
...
def _apply_python_noise_policy(findings: list[Finding], filename: str) -> list[Finding]:
    ...
    is_guardmarly_internal = _is_guardmarly_internal_python_path(filename)
    is_test = _is_test_file(filename)

    if not is_framework_internal and not is_guardmarly_internal and not is_test:
        return findings
    ...
    for finding in findings:
        ...
        # ── Guardmarly internal noise ───────────────────────────────────────
        if not reason and is_guardmarly_internal:
            if finding.rule_id in _GUARDMARLY_INTERNAL_PY_NOISE_RULES or _is_guardmarly_internal_python_noise_path(finding.rule_id, filename):
...
def _rule_29(ctx: _Ctx) -> list[Finding]:
    ...
    try:
        from guardmarly.cpg import build_cpg, CPGTaintEngine  # noqa: PLC0415
    except ImportError:
        return []
...
def _detect(code: str, filename: str = "", global_graph: object = None) -> list[Finding]:
    ...
    if has_non_regex_findings and ('if ' in code or 'assert ' in code):
        try:
            from guardmarly.engine.symbolic_guards import analyze_guards_python
    ...
    # A comment like  # guardmarly: ignore  or  # guardmarly: ignore[CWE-89]
...
def index_python_file(code: str, filename: str, global_graph):
    ...
    """
    import ast
    from guardmarly.ir.global_graph import NodeID, TaintNode, Edge
...
def analyze_python(code: str, filename: str = "", global_graph=None) -> AnalysisResult:
    ...
    try:
        from guardmarly.engine.rust_parser import HAS_RUST_CORE, fast_parse  # noqa: PLC0415
    except ImportError:
    ...
    try:
        from guardmarly.ssa_taint import run_ssa_taint_analysis
    ...
    try:
        from guardmarly.ir.stir import emit_python_stir
