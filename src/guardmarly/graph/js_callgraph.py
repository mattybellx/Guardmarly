...
from guardmarly.graph.import_graph import _ensure_file_node, _node_id_for_file, resolve_js_imports
...
def _parse_import_bindings(file_path: Path, content: str, root_dir: Path) -> tuple[dict[str, tuple[Path, str]], dict[str, Path]]:
    ...
    def _resolve(module_name: str) -> Path | None:
        from guardmarly.graph.import_graph import _load_tsconfig_aliases, _resolve_js_module
