...
Usage:
    from guardmarly.reporters import format_text, format_json, format_sarif
...
from guardmarly.schema import build_report
...
def format_text_multi(
    ...
    if console and colour:
        ...
        console.print(f"\n[bold]{'─' * 74}[/bold]")
        console.print(f"[bold cyan]  guardmarly[/bold cyan]  --  {len(results)} file(s) scanned")
    ...
    sep = "-" * 72
    parts.append(sep)
    parts.append(f"  guardmarly  --  {len(results)} file(s) scanned")
    parts.append(sep)
    parts.append("")
...
def format_ciso_report(results: list[AnalysisResult]) -> str:
    ...
    # Draw table
    table = Table(title="🏢 Guardmarly-Static Executive Risk Profile", title_justify="left", show_header=True, header_style="bold magenta")
...
def _enriched_properties(finding: Finding) -> dict[str, Any]:
    ...
    if finding.cwe:
        try:
            from guardmarly.engine.cvss import enrich_finding_properties
    ...
    for result in results:
        for f in result.findings:
            ...
            if rule_id not in rules_by_id:
                ...
                if f.cwe:
                    try:
                        from guardmarly.engine.cvss import get_cvss as _cvss_get, get_owasp as _owasp_get
        ...
        "runs": [{
            "tool": {
                "driver": {
                    ...
                    "informationUri": "https://github.com/mattybellx/Guardmarly",
...
<title>guardmarly Security Report</title>
<style>
  :root {{
...
<body>
<h1>&#x1F6E1; guardmarly — Security Report</h1>
...
{file_sections}
<footer>guardmarly &mdash; SAST engine &mdash; <a href="https://github.com/mattybellx/Guardmarly" style="color:#7fbfff">github.com/mattybellx/Guardmarly</a></footer>
<script>
  ...
  function exportSARIF() {{
    const sarif = {{
      version: '2.1.0',
      runs: [{{
        tool: {{ driver: {{ name: 'guardmarly', version: '{version}', informationUri: 'https://github.com/mattybellx/Guardmarly' }} }},
    ...
    a.download = 'guardmarly-report.sarif';
    a.click();
  }}
</script>
</body>
...
def format_html(results: list[AnalysisResult]) -> str:
    ...
    import datetime
    from guardmarly.engine_version import get_engine_version as _gev
