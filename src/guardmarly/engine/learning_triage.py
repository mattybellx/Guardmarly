...
───────────────────────────────
When developers use `# guardmarly: ignore` or `// guardmarly: ignore`, the tool
...
def _extract_suppression_context(
    ...
    suppressed = re.sub(r'#\s*guardmarly:\s*ignore.*$', '', suppressed).strip()
    suppressed = re.sub(r'//\s*guardmarly:\s*ignore.*$', '', suppressed).strip()
...
def learn_from_suppression(
    ...
    *,
    ...
    Call this when the scanner encounters `# guardmarly: ignore` or
    `// guardmarly: ignore` on a line that had a finding.
    """
    lines = code.splitlines()
