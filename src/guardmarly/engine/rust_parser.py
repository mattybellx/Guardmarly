"""guardmarly.engine.rust_parser — Bridge to native Rust/Tree-sitter parsing.

When the `guardmarly_rust_core` native module is built and available, uses
...
Usage:
    from guardmarly.engine.rust_parser import parse_to_dsl, fast_parse, fast_triage
...
try:
    from guardmarly_rust_core import is_available as _native_available, parse_code_dict as _native_parse
...
if HAS_RUST_CORE:
    try:
        from guardmarly_rust_core._core import parse_flat_table as _flat_table
        _FLAT_TABLE = _flat_table
    except ImportError:
        pass
    try:
        from guardmarly_rust_core._core import fast_triage as _fast_triage_fn
...
def _rust_nodes_to_dsl(nodes: list[dict[str, Any]]) -> Any:
    ...
    from guardmarly.dsl.engine import ASTNode as AN
