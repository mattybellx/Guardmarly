"""
guardmarly.engine.semgrep_transpiler
────────────────────────────────────────
DIR-3.2: Rule-to-Semgrep transpiler.

Converts guardmarly rule contracts into Semgrep-compatible YAML rules
...
Usage:
    from guardmarly.engine.semgrep_transpiler import transpile_rule
...
from guardmarly.rules import get_rule_contract

# ── Pattern mapping: guardmarly rule IDs → Semgrep patterns ──────────────────
# These are hand-curated mappings for high-value rules where the guardmarly
...
def _build_semgrep_rule(
    ...
    """Build a single Semgrep rule YAML string from an guardmarly rule mapping."""
    ...
    tags_yaml = _yaml_value(["guardmarly", f"cwe-{cwe.lower().replace('-', '')}" if cwe else "security"], 2)

    return f"""rules:
  - id: guardmarly-{rule_id.lower()}
    languages: [{lang_list}]
...
def transpile_rule(rule_id: str) -> str:
    """Transpile a single guardmarly rule to Semgrep YAML.

    Args:
        rule_id: The guardmarly rule ID (e.g. "PY-004", "JS-001").
...
def transpile_all() -> str:
    """Transpile all supported guardmarly rules to Semgrep YAML.
