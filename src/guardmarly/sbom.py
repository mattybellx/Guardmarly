...
Usage:
    from guardmarly.sbom import generate_sbom
    sbom_json = generate_sbom(workspace_root=Path("."), fmt="cyclonedx")

CLI integration:
    guardmarly src/ --sbom cyclonedx --sbom-output sbom.json
    guardmarly src/ --sbom spdx --sbom-output sbom.spdx.json
"""
    ...
    doc: dict[str, Any] = {
        ...
        "metadata": {
            ...
            "component": {
                ...
                "properties": [
                    {
                        "name": "guardmarly:scanner_version",
                        "value": get_engine_version(),
                    },
                    {
                        "name": "guardmarly:cwe_count",
                        "value": str(len(cwe_list)),
                    },
                    {
                        "name": "guardmarly:owasp_top10_coverage",
                        "value": f"{len(owasp_map)}/10",
                    },
                    {
                        "name": "guardmarly:nist_reference",
                        "value": "https://nvd.nist.gov/",
                    },
                    {
                        "name": "guardmarly:owasp_reference",
...
def _to_spdx(components: list[_Component], workspace_root: Path) -> dict[str, Any]:
    ...
    cwe_list, owasp_map = _compliance_metadata()
    doc_ns = f"https://guardmarly.dev/sbom/{uuid.uuid4()}"
    ...
    return {
        ...
        "creationInfo": {
            "created": _iso_now(),
            "creators": [f"Tool: guardmarly-{get_engine_version()}"],
...
def _compliance_metadata() -> tuple[list[str], dict[str, list[str]]]:
    ...
    try:
        from guardmarly.rules import list_rule_contracts, _COMPLIANCE_TAG_MAP
