    ...
    *,
...
) -> dict[int, list[_SourceMapSegment]]:
    line_map: dict[int, list[_SourceMapSegment]] = {}
    source_index = 0
    original_line = 0
    original_col = 0
    ...
    for local_line_index, line in enumerate(mappings_raw.split(';')):
        ...
        for segment in line.split(','):
            if not segment:
                continue
            fields = _decode_vlq_segment(segment)
            if not fields:
                continue
            ...
            if len(fields) >= 4:
                ...
                if 0 <= source_index < len(sources):
                    ...
                    line_map.setdefault(generated_line, []).append(_SourceMapSegment(
...
def _load_sourcemap_path(filename: str) -> Path | None:
    if not filename:
        return None
...
def _parse_sourcemap_segments(source_map_path: Path) -> dict[int, list[_SourceMapSegment]]:
    """Parse source-map mappings into generated line/column segments.

    Keeps all source-carrying segments per generated line so findings can be
    mapped using the closest segment at or before the finding column.
    """
    try:
...
def _remap_location(
    ...
    if not segments:
        return None
    # best segment at or before generated column; otherwise nearest first segment
    chosen = segments[0]
...
def _remap_findings_to_source_map(findings: list[Finding], filename: str) -> list[Finding]:
    if not filename:
        return findings
    source_map_path = _load_sourcemap_path(filename)
    if source_map_path is None:
        return findings
    line_map = _parse_sourcemap_segments(source_map_path)
    if not line_map:
        return findings
    ...
    for finding in findings:
        ...
        location = _remap_location(line_map, finding.line, max(0, trace_col - 1))
        if not location:
            # bundled finding without a trustworthy source mapping: suppress
            continue
...
def analyze_js_ast(code: str, filename: str = "", global_graph: object | None = None) -> AnalysisResult:
    ...
    source_map_path = _load_sourcemap_path(filename) if filename else None
    if minified.is_minified and source_map_path is None:
        # High-noise bundles without source maps are not actionable.
        fallback = analyze_js(code, filename)
        result.findings = filter_inline_suppressions(_remap_findings_to_source_map(dedup_findings(fallback.findings), filename), code)
        return result
    ...
    merged = _remap_findings_to_source_map(merged, filename)
