...
TAINT_SOURCES: dict[str, str] = {
    ...
    "request.params":         "Pyramid request parameters (GET+POST+path)",
    "request.json_body":      "Pyramid parsed JSON body",
    ...
    "urllib.request.urlopen": "remote HTTP response (may be attacker-controlled)",
    "socket.recv":            "network socket data",
    "socket.recvfrom":        "network socket data",
    "httpx.get":              "remote HTTP response (may be attacker-controlled)",
    "httpx.post":             "remote HTTP response (may be attacker-controlled)",
    "httpx.request":          "remote HTTP response (may be attacker-controlled)",
    "aiohttp.ClientSession.get": "remote HTTP response (may be attacker-controlled)",
    # XML / data format parsers (result may contain attacker data)
    "xml.etree.ElementTree.parse":  "parsed XML (may be attacker-controlled)",
    "lxml.etree.parse":             "parsed XML (may be attacker-controlled)",
    "defusedxml.ElementTree.parse": "parsed XML (may be attacker-controlled)",
    "xml.dom.minidom.parse":        "parsed XML (may be attacker-controlled)",
    "xml.sax.parseString":          "parsed XML (may be attacker-controlled)",
    "toml.load":                    "parsed TOML (may be attacker-controlled)",
    "tomllib.load":                 "parsed TOML (may be attacker-controlled)",
    "configparser.ConfigParser.read":"parsed config (may be attacker-controlled)",
    # Message queue / event sources
    "kafka.KafkaConsumer":   "Kafka message (external input)",
    "pika.BlockingConnection":"RabbitMQ message (external input)",
    "redis.Redis.get":       "Redis value (may be attacker-controlled)",
    "redis.StrictRedis.get": "Redis value (may be attacker-controlled)",
...
TAINT_SINKS: dict[str, _SinkInfo] = {
    ...
    # ZipSlip — CWE-22 path traversal via archive extraction
    "tarfile.extract":            ("CWE-22", "ZipSlip — Path Traversal via tarfile extract"),
    "tarfile.extractall":         ("CWE-22", "ZipSlip — Path Traversal via tarfile extractall"),
    "zipfile.ZipFile.extract":    ("CWE-22", "ZipSlip — Path Traversal via zipfile extract"),
    "zipfile.ZipFile.extractall": ("CWE-22", "ZipSlip — Path Traversal via zipfile extractall"),
    "shutil.unpack_archive":      ("CWE-22", "ZipSlip — Path Traversal via shutil unpack"),
