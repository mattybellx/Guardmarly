...
def sample_finding():
    return Finding(
        ...
        trace=(
            TraceFrame(kind="source", label="source", line=40),
            TraceFrame(kind="propagator", label="propagator", line=41),
            TraceFrame(kind="sink", label="sink", line=42),
        ),
    )
...
def sample_result(sample_finding):
    return AnalysisResult(file_path="app.py", language="python",
                          findings=[sample_finding], lines_scanned=50)


@pytest.fixture
def empty_result():
    return AnalysisResult(file_path="clean.py", language="python",
                          findings=[], lines_scanned=30)
...
def multi_finding_result():
    return AnalysisResult(file_path="mixed.py", language="python", findings=[
        Finding(category="security", severity=Severity.CRITICAL, title="CWE-89: SQLi", description="x",
                line=10, suggestion="x", rule_id="PY-004", cwe="CWE-89", agent="p", confidence=0.98, analysis_kind="taint-flow"),
        Finding(category="security", severity=Severity.HIGH, title="CWE-78: CMDi", description="x",
                line=25, suggestion="x", rule_id="PY-005", cwe="CWE-78", agent="p", confidence=0.92, analysis_kind="taint-flow"),
        Finding(category="security", severity=Severity.MEDIUM, title="CWE-327: Weak crypto", description="x",
                line=50, suggestion="x", rule_id="PY-013", cwe="CWE-327", agent="p", confidence=0.85, analysis_kind="pattern"),
        Finding(category="architecture", severity=Severity.LOW, title="Complexity", description="x",
                line=70, suggestion="x", rule_id="PY-044", agent="p", confidence=0.70, analysis_kind="metric"),
    ], lines_scanned=100)
...
def finding_no_trace():
    return Finding(category="security", severity=Severity.HIGH, title="CWE-798: Hardcoded secret",
                   ...
                   agent="p", confidence=0.95, analysis_kind="pattern")
...
def auto_fix_finding():
    return Finding(category="security", severity=Severity.HIGH, title="CWE-862: Missing auth",
                   ...
                   cwe="CWE-862", agent="p", confidence=0.88, analysis_kind="route_heuristic",
