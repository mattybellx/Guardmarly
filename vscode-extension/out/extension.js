...
function activate(context) {
    ...
    async function scanDocument(document) {
        ...
        try {
            ...
            const result = (0, child_process_1.execSync)(cli, {
            ...
            const diagnostics = latestFindings.map((f) => {
    ...
    // ── Gutter decorations ──────────────────────────────────────────────────
    function updateGutterDecorations(document, findings) {
    ...
    // ── Status bar updates ──────────────────────────────────────────────────
    function updateStatusBar(document, findingCount) {
        if (!cmdFound) {
    ...
    // ── Event listeners ─────────────────────────────────────────────────────
    context.subscriptions.push(vscode.workspace.onDidOpenTextDocument(scanDocument), vscode.workspace.onDidSaveTextDocument(scanDocument), vscode.window.onDidChangeActiveTextEditor(editor => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('guardmarly.showFindings', async () => {
        if (!cmdFound) {
            showInstallPrompt();
            return;
        }
    ...
    context.subscriptions.push(vscode.commands.registerCommand('guardmarly.scan', async () => {
        ...
        vscode.window.showInformationMessage(`Guardmarly: ${count} finding(s) in ${editor.document.fileName.split(/[\\/]/).pop()}`);
    }));
    context.subscriptions.push(vscode.commands.registerCommand('guardmarly.scanWorkspace', async () => {
        ...
        const spinInterval = setInterval(() => {
            if (scanning)
                statusBarItem.text = '$(sync~spin) Guardmarly: scanning workspace...';
        }, 200);
        try {
            const result = (0, child_process_1.execSync)(`${guardmarlyCmd} "${workspaceFolders[0].uri.fsPath}" --format text --fail-on never`, { encoding: 'utf-8', timeout: 120000, windowsHide: true });
            scanning = false;
            clearInterval(spinInterval);
    ...
    function showInstallPrompt() {
        vscode.window.showWarningMessage('Guardmarly CLI not found. Install it to enable security scanning.', 'Install with pip', 'View Docs').then(choice => {
            if (choice === 'Install with pip') {
                vscode.env.openExternal(vscode.Uri.parse('https://pypi.org/project/guardmarly/'));
            }
            else if (choice === 'View Docs') {
                vscode.env.openExternal(vscode.Uri.parse('https://github.com/mattybellx/Guardmarly#readme'));
            }
        });
    }
    // ── Initial scan ────────────────────────────────────────────────────────
...
function createColoredIconPath(context, color) {
